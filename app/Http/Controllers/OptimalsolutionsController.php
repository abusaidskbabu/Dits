...
use App\Models\Optimalsolutions;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect ; 


class OptimalsolutionsController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'optimalsolutions';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Optimalsolutions();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'optimalsolutions',
	...
	public static function display(  )
	{
		...
		$model  = new Optimalsolutions();
		$info = $model::makeInfo('optimalsolutions');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('optimalsolutions.public.view',$data);			
			}			
		} 
		else {
			...
			return view('optimalsolutions.public.index',$data);	
		}
