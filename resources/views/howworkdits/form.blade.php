...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'howworkdits?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> How word DITS</legend>
				{!! Form::hidden('id', $row['id']) !!}					
					...
					<select name='status' rows='5'   class='select2 '  > 
						...
						?></select> 
									  ...
									  </div> {!! Form::hidden('add_info', $row['add_info']) !!}{!! Form::hidden('created_at', $row['created_at']) !!}</fieldset></div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("howworkdits/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
