...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'ourtestimonials?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="col-md-12">
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						required     class='form-control form-control-sm ' /> 
									  ...
									  </div> {!! Form::hidden('work_in', $row['work_in']) !!}					
									  <div class="form-group row  " >
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/testimonial") !!}
									  ...
									  <div class="form-group row  " >
										<label for="Dialogue" class=" control-label col-md-4 text-left"> Dialogue <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <textarea name='dialogue' rows='5' id='dialogue' class='form-control form-control-sm '  
				         required  >{{ $row['dialogue'] }}</textarea> 
										 </div> 
										 <div class="col-md-2">
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("ourtestimonials/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
