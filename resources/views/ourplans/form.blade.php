...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'ourplans?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Our Plan</legend>
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/icons") !!}
						</div>
				         ...
				         required  >{{ $row['description'] }}</textarea> 
									  ...
									  </div> {!! Form::hidden('link', $row['link']) !!}{!! Form::hidden('add_info', $row['add_info']) !!}					
									  <div class="form-group row  " >
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("ourplans/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
