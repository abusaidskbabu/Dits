...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'aboutourself?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> About Ourself</legend>
				{!! Form::hidden('id', $row['id']) !!}					
									  <div class="form-group row  " >
										<label for="About" class=" control-label col-md-4 text-left"> About </label>
										<div class="col-md-6">
										  <textarea name='about' rows='5' id='editor' class='form-control form-control-sm editor '  
						 >{{ $row['about'] }}</textarea> 
										 </div> 
										 <div class="col-md-2">
						...
						<div class="fileUpload btn " > 
						    <span>  <i class="fa fa-camera"></i>  </span>
						    <div class="title"> Browse File </div>
						    <input type="file" name="who_we_are" class="upload"   accept="image/x-png,image/gif,image/jpeg"     />
						</div>
						<div class="who_we_are-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["who_we_are"],"/uploads/images") !!}
						</div>
						...
						<div class="fileUpload btn " > 
						    <span>  <i class="fa fa-camera"></i>  </span>
						    <div class="title"> Browse File </div>
						    <input type="file" name="what_out_value" class="upload"   accept="image/x-png,image/gif,image/jpeg"     />
						</div>
						<div class="what_out_value-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["what_out_value"],"/uploads/images") !!}
						</div>
									  ...
									  <div class="form-group row  " >
										<label for="Our Mission" class=" control-label col-md-4 text-left"> Our Mission </label>
										<div class="col-md-6">
										  <textarea name='our_mission' rows='5' id='our_mission' class='form-control form-control-sm '  
				           >{{ $row['our_mission'] }}</textarea> 
										 </div> 
										 <div class="col-md-2">
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('aboutourself/comboselect?filter=status:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("aboutourself/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
